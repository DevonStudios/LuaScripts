...
-- event.onmemorywrite(writeCheck, 0x02020000)
...
while true do
 ...
 elseif mode[index] == "100% Catch" then
  ...
  battleScreen = mdword(0x0600D000) ~= 0
  species = nationalDexTable[mword(0x02023528 + monInfo) + 1]
 ...
 elseif mode[index] == "Initial Seed Bot" then
  if botInstructions then
   gui.text(508, 1, "3 - Hide instructions")
   gui.text(2, 21, "1) Pause the game")
   gui.text(2, 41, "2) Reset the emulaor")
   gui.text(2, 61, "3) Advance one frame holding SELECT")
   gui.text(2, 81, "4) Unpause the game")
  else
   gui.text(508, 1, "4 - Show instructions")
  end
   ...
   while delay1 ~= delay5 and not found do
    while delay2 ~= delay4 and not found do
     while delay3 ~= delay6 and not found do
      ...
      joypad.set({A = true})
      i = 0
      while mword(0x02020010) == 0 and i < 200 do
       emu.frameadvance()
       i = i + 1
      end
      if mword(0x02020010) ~= 0 then
       initSeed = mword(0x02020000)
       found = isFound()
      end
      if not found then
  ...
  if found then
   ...
   gui.text(2, 120, "Initial Seed: "..string.format("%04X", initSeed))
