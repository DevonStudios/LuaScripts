...
function showInstructions()
 ...
 elseif mode[index] == "Initial Seed Bot" and showInstructionsText then
  gui.text(155, 1, "3 - Hide instructions")
  gui.text(1, 11, "1) Pause the game")
  gui.text(1, 21, "2) Reset the emulaor")
  gui.text(1, 31, "3) Advance one frame holding SELECT")
  gui.text(1, 41, "4) Unpause the game")
...
function showRngInfo(initial, current, frame)
 gui.text(0, 132, "Initial Seed: "..string.format("%04X", initial))
  ...
  while delay2 ~= delay4 and not initSeedFound do
   while delay3 ~= delay6 and not initSeedFound do
    ...
    if initSeedWritten then
     --print(string.format("%04X", mword(0x02020000)))
